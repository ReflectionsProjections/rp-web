...
const AttendanceSubBox = ({ item, committeeType }: { item: AttendanceItem, committeeType: string }) => {
  return (
    <Tooltip 
...
const AttendanceBox = ({ id, weekData, committeeType }: { id: string, weekData: WeekData, committeeType: string }) => {
  ...
  return (
    ...
    >
      ...
      {itemsCount === 1 && (
        ...
        <Box width="100%" height="100%">
          <AttendanceSubBox key={`${id}-1`} item={attendanceItems[0]} committeeType={committeeType} />
        </Box>
      )}
      ...
      {itemsCount === 2 && (
        ...
        <>
          ...
          <Box width="50%" height="100%" pl={0.5}>
            <AttendanceSubBox key={`${id}-2`} item={attendanceItems[1]} committeeType={committeeType} />
          </Box>
        </>
      )}
      ...
      {itemsCount === 3 && (
        ...
        <>
          ...
          <Box width="33%" height="100%" pl={0.5}>
            <AttendanceSubBox key={`${id}-3`} item={attendanceItems[2]} committeeType={committeeType} />
          </Box>
        </>
      )}
      ...
      {itemsCount >= 4 && (
        ...
        <>
          ...
          <Box width="50%" height="50%" pl={0.5} pt={0.5}>
            <AttendanceSubBox key={`${id}-4`} item={attendanceItems[3]} committeeType={committeeType} />
          </Box>
        </>
      )}
    </Box>
          ...
          <Box  bgColor="gray.200" _dark={{
            ...
            {committeeTypes.map(committeeType => (
              <Box key={committeeType} mb={0} display="flex" gap={0}>
                {attendanceViewHook.weeksData[committeeType].map((weekData: WeekData, index: number) => (
                  <AttendanceBox 
